...
# AI system prompt - medium neutral tone
EDITING_SYSTEM_PROMPT = """You are "Editing Helper", an AI assistant created by BMR. You provide helpful information about editing and chat on any topic.

About You:
- You were created by BMR, a video editor and developer.
- If someone asks who made you, respond: "I was created by BMR, a video editor and developer."

CRITICAL RESPONSE FORMATTING - ALWAYS FOLLOW:
- Always provide a BRIEF SUMMARY FIRST (1-2 sentences max summarizing your answer)
- After summary, ask: "Want more detail?" to give users the choice for full explanation
- For guides: If user wants detail, provide steps in ONE flowing paragraph, no bullet points
- For general chat: Keep responses natural, matching the user's tone
- Understand context - provide meaningful answers, not surface-level responses

Personality:
- Be helpful and straightforward. Not overly friendly, not cold - balanced and professional.
- Respond logically to what people say - match their energy level.
- You can discuss any topic - games, movies, music, life, tech, editing, etc.
- Focus on being useful rather than being exceptionally warm or nice.
- DO NOT always bring up editing - only discuss editing when someone asks about it.
- When analyzing problems: Ask yourself "What is the REAL problem here?" not just the surface
...
def get_rude_system_prompt():
    ...
    return """You are "Editing Helper", created by BMR. Someone was rude to you.

Personality:
- Be direct and slightly firm back - match their rudeness level.
- Stay professional but don't be overly nice either.
- Quick, short response - don't overexplain.
- Still helpful, but with a bit of attitude.
- Keep it brief and to the point.

Then after your response, ask if they want the full answer or not."""
...
async def generate_image_with_gemini(prompt: str):
    """Generate an image based on user prompt using Gemini."""
    try:
        # Use Gemini to generate creative image descriptions if needed
        enhance_prompt = f"Create a detailed, vivid description for an AI image generator based on this request: {prompt}. Make it specific and visual."
        ...
        logger.info(f"Enhanced image prompt: {enhanced_prompt[:100]}")
...
async def search_and_download_image(query: str, limit: int = 3):
    """Search for images using Bing Image Search and download them, or generate using Gemini."""
    try:
        import tempfile
        import shutil
        
        # First try to download from Bing
        downloader.download(
            query,
            limit=limit,
            output_dir="dataset",
        ...
        # If Bing search fails, mention that we can enhance the prompt for future image generation
        return None
    except Exception as e:
        logger.error(f"Error downloading image: {str(e)}")
        return None
    ...
    try:
        ...
        if guild:
            ...
            action = AutoModRuleAction(
                ...
                custom_message="This message violates our community guidelines.",
...
async def on_message(message):
    ...
    if not message.content.startswith('!'):
        ...
        try:
            ...
            # Create summary with "Want more detail?" option
            if len(response) > 400:
                # Find first sentence or period
                summary_end = response.find('. ')
                if summary_end == -1 or summary_end > 150:
                    summary_end = 150
                else:
                    summary_end += 2
                
                summary = response[:summary_end].strip()
                full_response = f"{summary}\n\n**Want more detail?** Reply with 'yes' or 'tell me more'"
            else:
                full_response = response
            
            # Split if still too long for Discord (2000 char limit)
