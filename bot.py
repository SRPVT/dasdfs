    ...
    try:
        ...
        if guild:
            ...
            try:
                ...
                logger.info(f"AutoMod rule created successfully: {rule.name}")
            except Exception as e:
                logger.warning(f"AutoMod rule creation failed (IGNORED): {e}")
...
async def unmute_command(ctx, member: discord.Member = None):
    """Remove timeout from a user - ONLY BMR can use this."""
    if not member:
    ...
    try:
        ...
        # Send DM to user
        try:
            await member.send(f"You have been **UNMUTED** in the server by {ctx.author.name}.")
        except:
            pass
...
# ====== ADVANCED MESSAGE & RESPONSE LOGGING (APPENDED) ======
# Logs every user message + every bot message sent via send() or reply().
...
# reuse existing send_live_log if present
try:
    send_live_log
except NameError:
    async def send_live_log(text: str):
        return

# --- Log every user message ---
async def _log_every_message(msg):
    if msg.author.bot:
        return
    try:
        preview = msg.content[:500]
        await send_live_log(f"📥 MESSAGE from {msg.author} ({msg.author.id}) in {msg.guild.name if msg.guild else 'DM'}:\n{preview}")
    except Exception as e:
        logger.debug(f"msg log err: {e}")

bot.add_listener(_log_every_message, "on_message")

# --- Monkeypatch send() and reply() to log bot responses ---
import discord.message

_original_send = discord.abc.Messageable.send
_original_reply = discord.Message.reply

async def _logged_send(self, content=None, *args, **kwargs):
    try:
        preview = (content or "")[:500]
        await send_live_log(f"📤 BOT SEND in {self}:\n{preview}")
    except Exception:
        pass
    return await _original_send(self, content, *args, **kwargs)

async def _logged_reply(self, content=None, *args, **kwargs):
    try:
        preview = (content or "")[:500]
        await send_live_log(f"📤 BOT REPLY to {self.author}:\n{preview}")
    except Exception:
        pass
    return await _original_reply(self, content, *args, **kwargs)

discord.abc.Messageable.send = _logged_send
discord.Message.reply = _logged_reply
