...
async def on_ready():
    ...
    # --- AutoMod Rule Creation (Global + Safe) ---
    try:
        from discord import (
            AutoModTrigger,
            AutoModRuleAction,
        ...
        for g in bot.guilds:
            try:
                # DELETE old duplicate rules
                existing_rules = await g.fetch_automod_rules()
                for rule in existing_rules:
                    if rule.name == "AutoMod Badge Rule":
                        await rule.delete()
                        logger.info(f"Deleted old AutoMod rule in {g.name}")

                # CREATE new clean rule
                ...
                logger.info(f"Created AutoMod rule successfully in {g.name}")

            except Exception as e:
                logger.warning(f"Failed to create AutoMod rule in {g.name}: {e}")

    except Exception as e:
        logger.warning(f"Global AutoMod system error: {e}")
(f"Failed to create rule in {g.name}: {e}")

    except Exception as e:
        logger.warning(f"Global AutoMod rule creation failed: {e}")
(f"AutoMod rule creation failed: {e}")
